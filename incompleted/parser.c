...
static int getLevel(Scope* scope) {
...
Type* compileLValue(void) {
  ...
  case OBJ_PARAMETER:
    ...
    if (var->paramAttrs->kind == PARAM_VALUE)
      genLA(getLevel(PARAMETER_SCOPE(var)), PARAMETER_OFFSET(var));
    else
      genLV(getLevel(PARAMETER_SCOPE(var)), PARAMETER_OFFSET(var));
    varType = var->paramAttrs->type;
    break;
  case OBJ_FUNCTION:
    // Push return slot address.
    genLA(getLevel(var->funcAttrs->scope), RETURN_VALUE_OFFSET);
...
Type* compileFactor(void) {
  ...
  case TK_IDENT:
	...
	genLV(getLevel(PARAMETER_SCOPE(obj)), PARAMETER_OFFSET(obj));
      else {
	genLV(getLevel(PARAMETER_SCOPE(obj)), PARAMETER_OFFSET(obj));
	genLI();
      }
...
Type* compileIndexes(Type* arrayType) {
  ...
  while (lookAhead->tokenType == SB_LSEL) {
    ...
    if (sizeOfType(arrayType->elementType) > 1) {
